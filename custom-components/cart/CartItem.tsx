...
export function CartItem({
  ...
  return (
    <div className={cn("flex gap-3 py-4 border-b border-slate-100 last:border-0", className)}>
      {/* Image */}
      <div className="relative h-18 w-18 sm:h-20 sm:w-20 rounded-lg overflow-hidden bg-slate-100 shrink-0">
        <Image src={imageUrl} alt={name} fill className="object-cover" sizes="80px" />
      </div>

      {/* Info */}
      <div className="flex-1 min-w-0 flex flex-col gap-2">
        <Paragraph size="sm" className="font-semibold text-slate-900 line-clamp-2 leading-snug">
          {name}
        </Paragraph>
        ...
        <div className="flex items-center justify-between mt-auto">
          <div className="flex items-center border border-slate-200 rounded-lg overflow-hidden">
            <button
              ...
              disabled={loading}
              className="flex items-center justify-center h-8 w-8 text-slate-600 hover:bg-slate-100 transition-colors disabled:opacity-40"
